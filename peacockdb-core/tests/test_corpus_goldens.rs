...
use peacockdb_core::test_support::{
    BUILD, COLUMNS, Capture, MEASURED_RUNS, MODES, Regeneration, RunMeta, SKIPPED,
    assert_section, cost_golden, cpu_golden, load_csv, merge_section, merged_text, mode_named,
    ordered_sections, over_cap, parse_node_line, parse_run_section, record_header, result_golden,
    rows_match_the_recipes, stem, testdata_root,
};
...
// --- the benchmark tree and the record ----------------------------------------
...
fn benchmark_files() -> Vec<PathBuf> {
    ...
    let mut files = Vec::new();
    walk(
        &testdata_root().join("benchmark-results"),
        &mut files,
    );
    files.sort();
    assert!(
...
/// the compiler.
#[test]
#[ignore = "the committed tree predates this harness; the plan's last task re-measures it"]
...
#[test]
#[ignore = "the committed record predates this harness; the plan's last task re-measures it"]
