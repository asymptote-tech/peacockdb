...
use crate::executor::{AbiCall, Measured, Measurements, RunReport};
use crate::wire::{RecipePlan, Seq};

use super::{BUILD, COLUMNS, RECORD_PATH_ENV, RunMeta, TIMING_MODE};

// The row builder and the plan reader serve the harness, which a rust-only build has not;
// the checker and the heading below serve `test_corpus_goldens` in every shape.
/// One row per call the run made, in the order the driver made them.
///
/// `nodes` is [`nodes_as_recorded`](crate::executor::nodes_as_recorded):
...
#[cfg_attr(feature = "rust-only", allow(dead_code))]
pub(crate) fn declared_steps(recipes: &RecipePlan) -> BTreeMap<usize, BTreeSet<Seq>> {
    (0..recipes.nodes())
        .map(|node| {
            let seqs = recipes
                .get(node)
                .map(|recipe| recipe.seqs())
                .unwrap_or_default();
...
pub(crate) fn rows_match_the_recipes(
    rows: &[String],
    ...
    for row in rows {
        ...
        match run {
            Some(first) if first != at => {
                return Err(format!(
                    "rows of executions {first} and {at} were checked together"
                ));
            }
            _ => run = Some(at),
        }
        ...
        if !calls.entry(seq).or_default().insert(call) {
            return Err(format!(
                "step #{seq} has two rows for call {call} — {row:?}"
            ));
        }
    }
...
pub(crate) fn record_header(meta: &RunMeta<'_>) -> String {
    format!(
        "{HEADER_NOTES}\n{}\n{}",
...
# mode = the planning mode, `tp4-sized` and the like. The same query
...
pub(crate) fn append_records(rows: &[String], meta: &RunMeta<'_>) {
    let Ok(path) = std::env::var(RECORD_PATH_ENV) else {
        return;
    };
