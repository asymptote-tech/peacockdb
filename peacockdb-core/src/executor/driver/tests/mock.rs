...
pub(crate) fn spec(rows: usize, bytes: usize) -> Spec {
    Spec { rows, bytes }
}
...
impl Script {
    pub(crate) fn source(mut self, table: &str, lanes: Vec<Vec<Spec>>) -> Self {
        self.sources.push((table.to_string(), lanes));
        self
    }

    pub(crate) fn with_exec(mut self, exec: ExecRule) -> Self {
        self.exec = exec;
        self
    }
    ...
    pub(crate) fn with_emit(mut self, emit: EmitRule) -> Self {
        self.emit = emit;
        self
    }

    pub(crate) fn with_join(mut self, join: JoinRule) -> Self {
        self.join = join;
        self
    }
    ...
    pub(crate) fn pricing_the_emission(mut self) -> Self {
        self.accumulator_prices_emission = true;
        self
    }

    pub(crate) fn failing_at(mut self, at: FailAt) -> Self {
        self.fails_at = Some(at);
        self
    }

    pub(crate) fn miswired(mut self) -> Self {
        self.miswired = true;
        self
    }

    pub(crate) fn measuring(mut self, scratch: usize) -> Self {
        self.measured_scratch = Some(scratch);
        self
    }

    pub(crate) fn modelling(mut self, per_byte: usize) -> Self {
...
/// half is taken.
pub(crate) fn address_of(node: &dyn GpuNode) -> usize {
    node as *const dyn GpuNode as *const () as usize
}

pub(crate) fn cpu_batch(rows: usize) -> CpuBatch {
