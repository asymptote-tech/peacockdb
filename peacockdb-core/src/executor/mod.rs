...
impl GpuBatch {
    ...
    pub(crate) fn producer(&self) -> Seq {
        self.producer
    }
...
// The journal below is filled by the device backend and read by the benchmark harness and
// the driver tests; a rust-only build has only the driver stamping it, so the lint is read
// where the fillers and the readers are. The same attribute sits on the measurement types.
...
pub(crate) struct AbiCalls(Option<Box<Vec<AbiCall>>>);

#[cfg_attr(feature = "rust-only", allow(dead_code))]
impl AbiCalls {
    /// Armed only for a measured run: unarmed, `record` is a branch and nothing else.
    pub(crate) fn armed(measuring: bool) -> Self {
        Self(measuring.then(Box::default))
    }

    pub(crate) fn record(&mut self, call: AbiCall) {
    ...
    pub(crate) fn recorded(&self) -> Option<&[AbiCall]> {
        self.0.as_deref().map(Vec::as_slice)
    }

    /// For the driver alone, to stamp `call_index` — see the field.
    pub(crate) fn recorded_mut(&mut self) -> Option<&mut [AbiCall]> {
...
pub struct RunReport {
    ...
    pub(crate) abi_calls: Vec<Vec<Vec<AbiCalls>>>,
...
impl Measurements {
    ...
    /// knowing its lengths.
    pub(crate) fn lanes(&self, node: usize) -> &[Vec<Option<Measured>>] {
        &self.per_entry[node]
    }
    ...
    pub(crate) fn call(&self, seq: Seq, call_index: u64) -> Option<Measured> {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match *self {
            ...
            RmmPool::Unavailable => {
                write!(
                    f,
                    "rmm-default (pool unavailable), cudaMalloc per allocation"
                )
            }
        }
    }
