...
- [ ] **Step 1: The failing test**, rust-only, in the same file's test module: call
...
- [ ] **Step 2:** At the top of `same_within_welford`, before the exact projection:
...
- [ ] **Step 3:** Both new tests green; `cargo test --features rust-only -p peacockdb-core
  --lib -- tests::gpu_tests::aggregate_cases` compiles (the device cases do not run rust-only).
- [ ] **Step 4: Commit.** `git commit -m "same_within_welford: names and types exact, values within tolerance on the finalize alone"`.
...
- [ ] **Step 1:** Restore the eight-row probe as a builder local to the case — the shape
...
- [ ] **Step 2:** Device cycle `PCK_TEST_FILTER='join_dimension_cases'`: the pin green (it
  asserts the divergence), everything else untouched. Add the case to #59's pin list in
  `tickets.md` and to the `bug_` table.
- [ ] **Step 3: Commit.** `git commit -m "#59 pinned on a composite key: a null in the second column matches on the device"`.
...
- [ ] **Step 1:** `ones` becomes a `UInt64Array` (or `Int64Array`, whichever `welford_state()`
...
- [ ] **Step 2:** Device cycle `PCK_TEST_FILTER='tests::gpu_tests::aggregate'`: the two
  ...
  fixture.
- [ ] **Step 3: Commit.** `git commit -m "welford_partial: a null value counts nothing, as an init would emit"`.

### Task 4: The record

- [ ] `build-test.md`: counts, the `bug_` table (+1, or more if Task 3 pinned). Detail files:
