...
turns the rule into a compiler check, and its warning count is the work list. The corpus harness
moves to `src/test_support/`, reached by the two external corpus binaries through three functions
whose signatures carry no engine type.
...
- Move: `llm-wiki/tasks/module-layout-baselines/{visibility-dump.py,case-inventory.sh,compare-inventory.sh}`
  → `scripts/`
- Create: `llm-wiki/tasks/test-support-baselines/{visibility-items.txt,inv-*.txt,goldens.sha256}`
...
Task 3 moved these three out of `module-layout-baselines/` because they are checks in both tasks.
...
CUDF_ROOT=~/data/miniforge3/envs/rapids-cuda-12.2 scripts/case-inventory.sh cudf \
  > llm-wiki/tasks/test-support-baselines/inv-cudf.txt
CUDF_ROOT=~/data/miniforge3/envs/rapids-cuda-12.2 scripts/case-inventory.sh gpu \
  > llm-wiki/tasks/test-support-baselines/inv-gpu.txt
```
...
### Task 4: Demote `plan/mod.rs` — 92 items, the largest
...
### Task 5: Demote `planner/mod.rs` — 7 items, 4 stay
...
### Task 6: Demote `executor/mod.rs` — 48 items, 2 stay
...
### Task 7: Demote `wire/mod.rs` — 20 items
...
feature as of Task 3, so their reason is gone.
...
### Task 8: Demote `plan_text/mod.rs` and `planner/translator/mod.rs` — 5 items
...
### Task 9: `common.rs`, `lib.rs`, and the last of the count
...
Task 3 emptied it. Delete the constant, both directions of its check, and the helper that parsed
...
### Task 11: `coding-style.md`, and the rest of the wiki
...
### Task 12: The residues, and two tickets
