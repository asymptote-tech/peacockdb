...
| Harness helpers | the synthetic batch and the comparator against themselves, with no engine: `synthetic(rows, seed)` deterministic from its seed, null in every column, dyadic in every float, legal at zero rows; `assert_same` red on a differing value, a differing type, a differing row count, and a slot one side left empty against a zero-row batch on the other — the failures the operator harness rests on, each shown for the reason it names | [tests::compare](../peacockdb-core/src/tests/compare.rs), [tests::synthetic](../peacockdb-core/src/tests/synthetic.rs) | 14 |
| *component* | | | |
