...
| **gpu — `--features gpu`: shad-gpu only. 89 cases: `--lib -- gpu_tests::` 81, `test_gpu_corpus` 8** | | | |
| *crate integration, external* | | | |
...
| *crate integration, internal* | | | |
| Operator harness | one hand-built node over stub leaves, a script of synthetic batches, both backends through `executors_for`, the outputs compared slot by slot and exactly — schema included, so a type the device changes is red here rather than at a query's root. The upload round trip, `GpuUnload` and `GpuLimit` are the cases here, the operators whose recipe carries no seq; a guard reads the kind each case declares and names every kind with none, thirteen pending for [`operator-cases.md`](tasks/operator-cases.md) | [an_unload_hands_the_whole_batch_over_on_both_backends](../peacockdb-core/src/tests/gpu_tests/harness_cases.rs), [every_kind_has_a_case_or_is_named_as_pending_or_excluded](../peacockdb-core/src/tests/gpu_tests/coverage.rs) | 26 |
| *component* | | | |
