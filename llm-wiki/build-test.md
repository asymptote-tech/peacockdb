...
| Plan-executor (C++) | hand-built plan IR through the C++ executor, node by node, plus the per-call entry points at their contract edges (row-group override, export range, slice), the sqrt arm on both evaluators, a merge that emits state rather than a value, and a typed null literal on the AST path | [PlanExecutor.HashJoinNationRegion](../cpp/tests/gpu/test_plan_executor.cpp), [Literals.AComparisonAgainstATypedNullKeepsNoRows](../cpp/tests/gpu/test_plan_executor.cpp) | shad-gpu | 29 |
